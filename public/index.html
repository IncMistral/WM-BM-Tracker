...
<button id="btnHelp" aria-label="Help - User Manual">Help</button>
</div>
...
function deleteGroup() {
  ...
  if (!contextTarget || contextTarget.groupIndex === undefined) return;
const gi = contextTarget.groupIndex;
const g = groups[gi];
...
applyPermissions();
  ...
  // ✅ Start keepalive video after login (user interaction)
  if (window.keepAwakeVideo) {
    window.keepAwakeVideo.play()
      .then(() => console.log('✅ Keepalive video started after login'))
        ...
        .map(p => {
          ...
          return `
            <div style="margin-bottom:1.5rem;">
              ...
              <p><strong>Start Date:</strong> ${p.startDate || "—"}<br>
   <strong>End Date:</strong> ${p.completionDate || "—"}</p>
              ${
                milestones
...
(function wireHelpButton() {
  ...
  if (btnHelp) {
    btnHelp.onclick = () => {
      // Create a temporary link to download the user manual
      const link = document.createElement('a');
      link.href = './BM_Project_Tracker_User_Manual.docx';
      link.download = 'BM_Project_Tracker_User_Manual.docx';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    };
...
setTimeout(function() {
  ...
  if (btnHelp) {
    btnHelp.addEventListener('click', function(e) {
      e.stopPropagation();
      e.preventDefault();
      window.location.href = '/BM_Project_Tracker_User_Manual.docx';
    });
  }
}, 1000);
