...
<head>
  ...
  <style>
    ...
    #topBar{
      display:flex; align-items:center; gap:0.75rem; padding:0.625rem 2.5rem;
...
<div id="loginModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="loginModalTitle">
 ...
 <div class="row">
  <div style="flex: 1;">
    ...
    <input type="text" id="projectNameInput" placeholder="Project name" style="width: 90%; margin: 0.3125rem auto; padding: 0.375rem; font-size: var(--fs-14); background: #222; color: var(--text); border: 1px solid var(--border); border-radius: var(--radius-sm); display: block;" />
  </div>
  <div style="flex: 1;">
    <label for="projectPOInput" style="display: block; margin-bottom: 0.25rem; color: var(--text); font-size: var(--fs-12);">PO Number</label>
    <input type="text" id="projectPOInput" placeholder="PO Number" style="width: 90%; margin: 0.3125rem auto; padding: 0.375rem; font-size: var(--fs-14); background: #222; color: var(--text); border: 1px solid var(--border); border-radius: var(--radius-sm); display: block;" />
  </div>
</div>
      <div class="row">
  <div style="flex: 1;">
...
function addLogEntry(groupName, type, details) {
  ...
  if (groupName && groupName.trim()) {
    ...
    if (found) {
      safeGroup = found.name;   // ✅ use the current official group name
    } else {
  ...
  // ✅ Post a single log entry to the backend
  fetch("/api/logs", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function compileDailyLogs() {
  ...
  // ✅ Render logs into daily log panel
...
groupLabel.style.left = (sidePad + 10) + "px";   // 👈 label’s X position
...
// The group name is baked into this page
        ...
        // Fetch fresh logs from server instead of using stale local data
        let logs = [];
        
        // Fetch logs from server on page load
        async function loadFreshLogs() {
          try {
            const res = await fetch("/api/logs");
            logs = await res.json();
            console.log("Loaded fresh logs from server:", logs.length);
          } catch (err) {
...
document.getElementById("btnViewLogs").onclick = () => {
        ...
        document.addEventListener("DOMContentLoaded", async () => {
          await loadFreshLogs(); // Load fresh logs first
          refreshLogDropdown(groupName);
          console.log("✅ Dropdown initialized for", groupName);
        });
...
compileBtn.style.left = (sidePad + 105) + "px";  // 👈 adjust this number to set distance from left edge
...
if (p.poNumber) {
  ...
  poNumber.style.left = sidePad + "px";
  poNumber.style.top = (y + BAR_HEIGHT/2 - 5) + "px";  // Position below project name
  poNumber.style.width = labelColWidth + "px";
  poNumber.style.textAlign = "right";
  poNumber.style.fontSize = "0.75rem";
  poNumber.style.color = "#ccc";  // Slightly dimmed color
  poNumber.textContent = "PO: " + p.poNumber;
  ...
  if (p.startDate) {
    ...
    startLabel.style.left = (barXBase + 65) + "px";
...
// 🔧 SIMPLE TV FIX - Direct screen width detection with debug
...
if (Array.isArray(p.milestones)) {
  p.milestones.forEach(m => {
    ...
    if (p.completionDate) {
      ...
      comp.style.left = (barX + barW - 120) + "px";
      comp.style.top = (y - 16) + "px";
      comp.style.fontSize = "0.8rem";
...
function wireToolbar() {
  ...
  if (btnNewGroup) {
    btnNewGroup.onclick = () => {
      // ✅ use the logging-enabled newGroup() function
      newGroup();
    };
  }
...
(function wireEditUserButton() {
  ...
  // Start auto-refresh if user is already logged in on page load
  startAutoRefresh();
